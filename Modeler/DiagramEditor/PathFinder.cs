...
namespace DiagramDesigner {
  ...
  internal class PathFinder {
    ...
    internal static List<Point> GetConnectionLine(ConnectorInfo source, ConnectorInfo sink, bool showLastLine, bool simpleLine,
      ArrowSymbol endSymbol, ArrowSymbol startSymbol) {
      ...
      //var startPoint = source.Position;
      //var endPoint = sink.Position;
      if ((simpleLine) || (source.Orientation == ConnectorOrientation.None) || (sink.Orientation == ConnectorOrientation.None)) {

        var angle = Math.Atan2(sink.Position.Y - source.Position.Y, sink.Position.X - source.Position.X);
        const int arrowSize = 8;
        double dxEnd = 0;
        double dyEnd = 0;
        if (endSymbol != ArrowSymbol.None) {
          dxEnd = arrowSize * Math.Cos(angle);
          dyEnd = arrowSize * Math.Sin(angle);
        }
        double dxStart = 0;
        double dyStart = 0;
        if (startSymbol != ArrowSymbol.None) {
          dxStart = arrowSize * Math.Cos(angle);
          dyStart = arrowSize * Math.Sin(angle);
        }
        return new List<Point> {
          new Point(source.Position.X + dxStart, source.Position.Y + dyStart), 
          new Point(sink.Position.X - dxEnd, sink.Position.Y - dyEnd) };// sink.Position };
      }
    ...
    internal static List<Point> GetConnectionLine(ConnectorInfo source, Point sinkPoint, ConnectorOrientation preferredOrientation, bool isSimple) {
      if ((isSimple) || (source.Orientation == ConnectorOrientation.None)) {

        return new List<Point>() { source.Position, sinkPoint };
      }
    ...
    private static void GetOppositeCorners(ConnectorOrientation orientation, Rect rect, out Point n1, out Point n2) {
      switch (orientation) {
        ...
        case ConnectorOrientation.Bottom:
          ...
          break;
        default:
          n1 = new Point(rect.Left + rect.Width / 2, rect.Top + rect.Height / 2);
          n2 = n1;
          break;
      }
    }
